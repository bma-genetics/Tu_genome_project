...
use strict;
BEGIN{
      push(@INC,'/public/share/hldu/wheat_bac_pacbio/script_pipeline/');
}
...
while(<IN3>){
      ...
      for(my $i=0;$i<@content;$i=$i+6){
          ...
          if($content[$i+1]==0){       #contig no reverse
               if($infor_ctg[1]==0){   #
                  print OUT "$content[$i] $content[$i+1] 0 $infor_ctg[3] $infor_ctg[0] $content[$i+2] ";
                  if($content[$i+2]==0){ #bac no reverse 
                         print OUT "$infor_ctg[10] ";
                  }
                  elsif($content[$i+2]==1){ #bac reverse
                         my $bac_start=$infor_ctg[8]-$infor_ctg[9];    #反链 坐标转换
                         
                         print OUT "$bac_start ";
                  }
               }
               elsif($infor_ctg[1]==1){
                  my $ctg_end=$ctg_len{$content[$i]}-$infor_ctg[2];
                  print OUT "$content[$i] $content[$i+1] 0 $ctg_end $infor_ctg[0] $content[$i+2] ";
                  if($content[$i+2]==0){
                         print OUT "$infor_ctg[10] ";
                  }
                  elsif($content[$i+2]==1){
                         my $bac_start=$infor_ctg[8]-$infor_ctg[9];
                         print OUT "$bac_start ";
                  }
               }
          }
